...
use crate::ast::michelson_address::{FromBase58CheckErrorEq, FromBytesErrorEq};
use crate::ast::*;
...
impl ContractScript<ParsedStage> {
    ...
    pub fn typecheck(
        ...
        let code = self.code.typecheck(ctx, &parameter, &mut stack)?;
        unify_stacks(
            ctx,
...
fn typecheck_value(ctx: &mut Ctx, t: &Type, v: Value) -> Result<TypedValue, TcError> {
    ...
    Ok(match (t, v) {
        ...
        (T::Or(pt), V::Left(lv)) => {
            let (tl, _) = pt.as_ref();
            let lv = typecheck_value(ctx, tl, *lv)?;
            TV::new_or(crate::ast::Or::Left(lv))
        }
        (T::Or(pt), V::Right(rv)) => {
            let (_, tr) = pt.as_ref();
            let rv = typecheck_value(ctx, tr, *rv)?;
            TV::new_or(crate::ast::Or::Right(rv))
        }
...
mod typecheck_tests {
    ...
    fn typecheck_instruction(
        ...
        super::typecheck_instruction(i, ctx, &Type::Unit, opt_stack)
    }
