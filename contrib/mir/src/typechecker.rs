...
fn parse_ty_with_entrypoints(
    ...
    let parsed_ty = match ty {
        ...
        App(ticket, [t], _) => {
            ...
            // See https://mavryk-network.gitlab.io/michelson-reference/#type-ticket
        ...
        App(contract, [t], _) => {
            ...
            // See https://mavryk-network.gitlab.io/michelson-reference/#type-contract
...
pub(crate) fn typecheck_instruction<'a>(
    ...
    Ok(match (i, stack.as_slice()) {
        ...
        (App(FAILWITH, [], _), [.., _]) => {
            ...
            // https://mavryk-network.gitlab.io/michelson-reference/#instr-FAILWITH claim
        ...
        (App(EMIT, [t], anns), [.., _]) => {
            ...
            // https://mavryk-network.gitlab.io/michelson-reference/#instr-EMIT claim
            // the type needs to be packable, but that's not quite correct, as
            // `contract _` is forbidden. The correct constraint is seemingly
            // "pushable", as "pushable" is just "packable" without `contract _`
    ...
    Ok(match (t, v) {
        ...
        (T::BigMap(m), v) => {
            let (id_opt, vs_opt) = match v {
                ...
                // https://mavryk-network.gitlab.io/michelson-reference/#type-big_map
