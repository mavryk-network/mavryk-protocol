...
let operation_status :
    ([`GET], unit, unit, op_query, unit, status option) Mavryk_rpc.Service.t =
  Mavryk_rpc.Service.get_service
    ...
    Mavryk_rpc.Path.(root / "operation_status")

let inject : ([`GET], unit, unit, unit, unit, unit) Mavryk_rpc.Service.t =
  Mavryk_rpc.Service.get_service
    ~description:"Inject operations"
    ~query:Mavryk_rpc.Query.empty
    ~output:Data_encoding.unit
    Mavryk_rpc.Path.(root / "inject")
