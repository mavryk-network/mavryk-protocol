...
Another pre-requisite for building the documentation is making sure that the Mavkit sources on your branch are compiled, because part of the documentation is generated by Mavkit executables.
...
Mavkit generates the API documentation for all libraries in HTML format. The
...
The documentation is not installed on the system by Mavkit. It is meant to be
...
Typically, Mavkit installation scripts not only have to evolve with the Mavkit codebase, but also with various other evolving resources, such as OPAM packages, package managers, Linux distributions, and so on.
...
For example, the following directive includes a script fragment detailing a step in compiling the Mavkit sources::
