...
``MAVRYK_PROTOCOL`` that denote these layers of abstraction.
...
come just before ``Storage`` in ``MAVRYK_PROTOCOL``.
...
automatic documentation generation possible (as in ``mavkit-client rpc
