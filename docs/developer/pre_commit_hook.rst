...
Pre-commit is configured via a simple configuration file :src:`.pre-commit-config.yaml` that is kept in sync by the Mavryk developers with the best practice used on the Mavryk repository.
