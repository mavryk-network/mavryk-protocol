...
The Mavkit node can be configured in flexible ways to control various
...
- by creating and updating the configuration file using the ``config`` command of ``mavkit-node``. This covers a subset of the CLI of the ``run`` command of ``mavkit-node`` mentioned above.

  The list of parameters configurable via the ``config`` command can be obtained using the following command::

    mavkit-node config update --help

- by directly editing the configuration file. This allows to specify all the available configuration parameters, including some that cannot be set using the options of the ``config`` and ``run`` commands of ``mavkit-node``, for example network parameters such as connection or authentication timeouts.

  The list of configurable parameters in the configuration file can be obtained using the following command::

    mavkit-node config --help
...
::
   ...
   mavkit-node config update <…>
   # Check your new values:
   mavkit-node config show
   # If you want to restart from an empty cfg file:
   mavkit-node config reset <…>
...
white-listed. Each element in the list is an API-endpoint (that can be passed to e.g. the ``mavkit-client rpc``
...
Additionally ``--allow-all-rpc`` CLI option to ``mavkit-node`` can be used to
simply allow all RPC endpoints on a given address. When passed to
``mavkit-node config`` command, this option modifies the ``config.json`` file,
putting an appropriate ACL there. When passed to ``mavkit-node run``, it
...
::
  ...
  $ mavkit-node run --rpc-addr localhost:8732
...
::
    ...
    $ mavkit-node run --rpc-addr localhost:8732 --rpc-addr 192.168.0.3:8732
...
be tempting to listen on the special ``0.0.0.0`` address::
  ...
  $ mavkit-node run --rpc-addr 0.0.0.0:8732
...
default, it is sufficient to open another listening address::
  ...
  $ mavkit-node run --rpc-addr localhost --rpc-addr 0.0.0.0

The ``--allow-all-rpc`` switch can be used to open all RPCs on a specific address::

  $ mavkit-node run --rpc-addr 192.168.0.3 --allow-all-rpc 192.168.0.3
...
::
   ...
   mavkit-node run --rpc-addr [::] --private-mode \
                                  --no-bootstrap-peers \
