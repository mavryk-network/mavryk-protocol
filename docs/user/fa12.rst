...
This page does not present FA1.2 tokens in general, but their Michelson interface and their support in the Mavkit client.

For a more general presentation of token standards such as FA1.2 from a Mavryk developer perspective, using higher-level smart contract languages, see `Token standards in Mavryk <https://docs.tezos.com/architecture/tokens#token-standards>`__.
...
<https://gitlab.com/tezos/tzip/-/blob/master/proposals/tzip-7/ManagedLedger.mv>`_.
...
The ``mavkit-client`` supports this standard with specific commands that allow
...
``mavkit-client man fa1.2`` gives a complete list of the `built-in commands
<https://protocol.mavryk.org/alpha/cli-commands.html#client-manual>`_ to
...
``mavkit-client check contract <alias or KT1> implements fa1.2``. The underlying
...
- ``mavkit-client from fa1.2 contract <fa1.2> transfer <amount> from
...
- ``mavkit-client from fa1.2 contract <fa1.2> approve <amount> as
...
As for mav, ``mavkit-client`` supports batch transfer of tokens from a single
source to multiple recipients on multiple FA1.2 contracts, using the command
``mavkit-client multiple fa1.2 transfers from <account> using <json>``.
...
.. code-block:: javascript
   ...
   [{ "token_contract": <string>, // address or alias of the FA1.2 contract
      ...
      "tez_amount": <string>, // (optional) amount of mav to send with transaction as a string
...
The complete schema can be inspected via ``mavkit-codec describe
<protocol_name>.fa1.2.token_transfer json schema`` (where ``<protocol_name>``
can be replaced with e.g. ``alpha`` or ``001-PtAtLas``).
...
``mavkit-client`` supports calling these entrypoints completely
offchain. One can get a balance using ``mavkit-client from fa1.2
contract <contract> get balance for <account>``, or the allowance
using ``mavkit-client from fa1.2 contract <contract> get allowance on
<owner> as <operator>``.
