...
- A new ``SUB_MUMAV`` instruction has been added, it is similar to the
  ``mumav`` case of the ``SUB`` instruction but its return type is
  ``option mumav`` instead of ``mumav``. This allows subtracting
  ``mumav`` values without failing in case of underflow. (MR :gl:`!3079`)

- The ``SUB`` instruction on type ``mumav`` is deprecated. It can be
  replaced by ``SUB_MUMAV; ASSERT_SOME`` (and ``SUB; DROP`` can be
