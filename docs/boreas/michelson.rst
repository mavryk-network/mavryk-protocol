...
the :ref:`concrete syntax <ConcreteSyntax_boreas>`. In particular
...
-  ``CREATE_CONTRACT { parameter 'p ; storage 'g ; code ... }``:
   ...
   There is a new version of this instruction, see its `documentation <https://protocol.mavryk.org/michelson-reference/#instr-CREATE_CONTRACT>`__.
...
:ref:`special annotations <SpecialAnnotations_boreas>` and are not allowed everywhere.
...
The multisig commands of :ref:`Mavkit command line client <client_manual_boreas>`
use this
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_boreas>`).
...
:ref:`syntax of errors <syntax_of_errors_boreas>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_boreas>` can
...
particular order.
 ...
 - ``other_contracts`` (optional, defaults to ``{}``): mapping between
   ...
   specifications <syntax_of_other_contracts_boreas>`)
...
.. _syntax_of_concrete_stacks_boreas:
...
.. _omitting_parts_of_the_output_boreas:
...
<syntax_of_concrete_operations_boreas>`) or implementation-specific
parts of error outputs (see the :ref:`syntax of errors
<syntax_of_errors_boreas>`).

.. _output_normalization_boreas:
...
:ref:`wildcard pattern <omitting_parts_of_the_output_boreas>` in the
...
raise:
 ...
 - ``(StaticError <error description>)``: an error occurred before the
   ...
   <omitting_parts_of_the_output_boreas>` such as ``(StaticError _)``
 ...
 - ``Overflow``: an overflow was detected. This can happen when an
   addition or multiplication on type ``mumav`` produces a result
   which is too large to be represented as a value of type ``mumav``,
...
.. _syntax_of_concrete_operations_boreas:
...
<omitting_parts_of_the_output_boreas>`.
...
.. _syntax_of_extra_big_maps_boreas:
