...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_boreas`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_boreas`).

.. _prevalidator_filters_boreas:
...
on the :ref:`validation pass<validation_passes_boreas>`.Some extra information (like the fees, or the gas/fees
