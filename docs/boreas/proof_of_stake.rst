...
<delegating_coins>`. Therefore, in Mavryk, it is the :ref:`delegates<def_delegate_boreas>`
...
The PRNG's :ref:`seeds <random_seed_boreas>` are obtained from random
...
becomes effective after ``CONSENSUS_RIGHTS_DELAY + 2`` :ref:`cycles <def_cycle_boreas>`.  The
value ``CONSENSUS_RIGHTS_DELAY`` is a :ref:`protocol constant
<protocol_constants_boreas>`.
...
<protocol_constants_boreas>` and a minimal frozen stake given by the
...
If the :ref:`adaptive issuance <adaptive_issuance_boreas>`
...
the :ref:`stakes<active_stake_boreas>` of a specific cycle: the probability to sample a
...
:ref:`random seed<random_seed_boreas>` for the cycle to which the
level belongs, and the slot.


.. _ps_constants_boreas:
...
The adaptive issuance feature :ref:`documentation <adaptive_issuance_boreas>`.

Other presentations of the Mavryk' proof-of-stake mechanism can be
found in the
`Mavryk Documentation entry <https://documentation.mavryk.org/tezos-basics/liquid-proof-of-stake/>`_.
