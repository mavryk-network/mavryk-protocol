...
At the end of each blockchain :ref:`cycle <def_cycle_boreas>`, the
...
The :ref:`adaptive issuance rate <adaptive_rate_boreas>` determines, at the end
of cycle :math:`\IL{c}`, the issuance for cycle :math:`\IL{c + 3}`. The
adaptive issuance rate is the sum of a :ref:`static rate <static_rate_boreas>`
and a :ref:`dynamic rate <dynamic_rate_boreas>`. The final result is clipped to
...
proposed mechanism, the :ref:`adaptive issuance rate <adaptive_rate_boreas>`
...
rewards, in proportion to their relative :ref:`weights <reward_weights_boreas>`.
...
for contributing to :ref:`random seed generation <randomness_generation_boreas>`
...
endpoint <https://protocol.mavryk.org/boreas/rpc.html#get-block-id-context-issuance-expected-issuance>`__,
...
complementary to the existing :ref:`delegate <def_delegate_boreas>`
...
subject to :ref:`slashing <slashing_boreas>` if
the delegate misbehaves by :ref:`double-signing <def_double_signing_boreas>`
...
policy <staking_policy_configuration_boreas>` by setting staking parameters
...
As :ref:`participation rewards <adaptive_rewards_boreas>` are
...
4 cycles (cf. :ref:`Staked funds management <staked_funds_management_boreas>`).
...
the same name introduced for :ref:`implicit accounts <def_implicit_account_boreas>`.
...
activation <feature_activation_boreas>`: only
...
.. _staking_policy_configuration_boreas:
...
.. _staked_funds_management_boreas:
...
-  Changes to protocol constants. Note that this entails calculating
   ...
   values when :ref:`Adaptive Issuance <adaptive_issuance_boreas>` is not active.
...
formula <adaptive_rewards_boreas>` and keeps using the current reward
values.
