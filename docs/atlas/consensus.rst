...
Assuming ``blocks_per_minute = 4``, ``total_rewards`` is 20 mav.
We define:
...
Thus, we obtain ``BAKING_REWARD_FIXED_PORTION = 5`` mav,
(maximum) ``bonus = 5`` mav, and ``attesting_rewards = 10`` mav.
...
``5 / (7000 / 3) = 0.002143`` mav and an attesting
rewards per slot of ``10 / 7000 = 0.001428`` mav.
...
Therefore B receives the bonus ``(5251 - 4667) * 0.002143 = 1.251512`` mav. (Note
...
reward of ``2,867,200 * 0.001428 = 4094.3616`` mav for that cycle.
...
.. list-table::
   ...
   * - ``BAKING_REWARD_BONUS_PER_SLOT``
     - ``bonus / (CONSENSUS_COMMITTEE_SIZE / 3)`` = 0.002143 mav
   * - ``ATTESTING_REWARD_PER_SLOT``
     - ``attesting_reward / CONSENSUS_COMMITTEE_SIZE`` = 0.001428 mav
