...
the :ref:`concrete syntax <ConcreteSyntax_atlas>`. In particular
...
:ref:`special annotations <SpecialAnnotations_atlas>` and are not allowed everywhere.
...
The multisig commands of :ref:`Mavkit command line client <client_manual_atlas>`
use this
...
:ref:`syntax of concrete stacks <syntax_of_concrete_stacks_atlas>`).
...
:ref:`syntax of errors <syntax_of_errors_atlas>`). For executions
expected to fail, the argument is the expected error. In both cases,
the :ref:`wildcard pattern <omitting_parts_of_the_output_atlas>` can
...
particular order.
 ...
 - ``other_contracts`` (optional, defaults to ``{}``): mapping between
   ...
   specifications <syntax_of_other_contracts_atlas>`)
 ...
 - ``big_maps`` (optional, defaults to ``{}``): mapping between
   ...
   <syntax_of_extra_big_maps_atlas>`)
...
<syntax_of_concrete_operations_atlas>`) or implementation-specific
...
:ref:`wildcard pattern <omitting_parts_of_the_output_atlas>` in the
...
raise:
 ...
 - ``(StaticError <error description>)``: an error occurred before the
   ...
   <omitting_parts_of_the_output_atlas>` such as ``(StaticError _)``
...
<omitting_parts_of_the_output_atlas>`.
