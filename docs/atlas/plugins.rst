...
:src:`src/proto_001_PtAtLas/lib_plugin/plugin.ml`. Thus, a specific version
...
- perform protocol-dependent computations that require data not available
  ...
  to reason on timestamps (see :ref:`consensus_filter_atlas`);
...
- implement some common operations that are customized for each
  protocol (e.g., :ref:`prevalidator_filters_atlas`).

.. _prevalidator_filters_atlas:
...
<mavkit-libs/Tezos_base/Mempool/index.html>`.
...
on the :ref:`validation pass<validation_passes_atlas>`.Some extra information (like the fees, or the gas/fees
