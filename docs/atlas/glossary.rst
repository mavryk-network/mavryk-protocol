...
_`Double signing`
    ...
    part of the double signer's deposit -- see :ref:`Slashing<slashing_atlas>`.

_`Failing Noop`
    The ``Failing_noop`` operation implements a *No-op*, which always
    fails at :ref:`application time<operation_validity_atlas>`, and
    should never appear in :ref:`applied
    blocks<full_application_atlas>`. This operation allows end-users to
...
_`Fee`
    ...
    Currently, only :ref:`manager operations<manager_operations_atlas>`
...
_`Stake`
    ...
    :ref:`here<active_stake_atlas>` for details.
...
_`Validation pass`
    ...
    application<operation_validity_atlas>` of certain classes of
    operations.
