...
for test_case in "$@"; do
  ...
  "install-opam-focal")
    ...
    ;;
  "compile-release-sources-bullseye")
    docker run --rm -i -v "$DOCS_DIR/introduction":/Scripts ocaml/opam:debian-11 /Scripts/compile-sources.sh mavryk-network/mavryk-protocol latest-release
    ;;
  "compile-sources-bullseye")
    docker run --rm -i -v "$DOCS_DIR/introduction":/Scripts ocaml/opam:debian-11 /Scripts/compile-sources.sh mavryk-network/mavryk-protocol master
    ;;
  "compile-sources-mantic")
    docker run --rm -i -v "$DOCS_DIR/introduction":/Scripts ocaml/opam:ubuntu-23.10 /Scripts/compile-sources.sh mavryk-network/mavryk-protocol master
    ;;
  "install-python-focal")
