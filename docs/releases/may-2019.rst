...
storage directory ``~/.mavryk-node``, begin by gracefully stopping the
node.
Back up your storage and upgrade it, it will only take a few seconds::

  cp -r ~/.mavryk-node mavryk-node-$(date +%Y%m%d)
...
We can proceed to export a snapshot::
  ...
  mavryk-node snapshot export mainnet-$(date +%Y%m%d).full
...
Alternatively we can also specify the block to snapshot::
  ...
  mavryk-node snapshot export mainnet-$HASH.full --block $HASH
...
(without forgetting, if you need, to save ``{identity,peers,config}.json``)::
    ...
    rm -r ~/.mavryk-node
    mavryk-node snapshot import ./mainnet-*.full

There should be a rather smaller ``~/.mavryk-node`` directory populated
...
- Commits:
    ...
    - ``333782792``: ``Alpha/Daemons: use the mavryk-client context to select the chain``
...
- Fixes
- Commits:
    - ``d7ba8bb07``: ``Resto: Fix #480: Keep the host: set by the user with MAVRYK_SIGNER_HTTP_HEADERS``
...
- Commits:
    ...
    - ``30816ed09``: ``Build,CI: Add the mavryk-event-logging library``
    ...
    - ``e5c396997``: ``[bin client]: remove wrong dependency to mavryk-shell``
    ...
    - ``d6a3b46f1``: ``Opam packaging: install MAVRYK_PROTOCOL as part of mavryk-embedded-protocol-%.raw``
    - ``4511710d4``: ``Flextesa: voting command takes the MAVRYK_PROTOCOL file as argument``
    - ``b6619de3a``: ``lib_network_sandbox: Make it 32bits arch compatible``
    - ``6e91d5ec2``: ``Opam packaging: fix mavryk-client-alpha-commands deps``
