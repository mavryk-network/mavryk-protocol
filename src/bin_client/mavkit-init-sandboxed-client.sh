...
BOOTSTRAP1_IDENTITY="mv18Cw7psUrAAPBpXYd9CtCpHg9EgjHP9KTe"
...
BOOTSTRAP2_IDENTITY="mv1V73YiKvinVumxwvYWjCZBoT44wqBNhta7"
...
BOOTSTRAP3_IDENTITY="mv1TxMEnmav51G1Hwcib1rBnBeniDMgG8nkJ"
...
BOOTSTRAP4_IDENTITY="mv1PVMnW8iyYxCoqLfPAha8EAPRxjTx7wqbn"
...
BOOTSTRAP5_IDENTITY="mv1S14SxfuavHMGDXxZJoBERZafLTyX3Z6Dx"
...
main () {
    ...
    if [ $(basename "$bin_dir") = "bin_client" ]; then
        ...
        # we assume a clean install with mavkit-(admin-)client in the path
        local_client="${local_client:-$(which mavkit-client)}"
        local_admin_client="${local_admin_client:-$(which mavkit-admin-client)}"
        local_signer="${local_signer:-$(which mavkit-signer)}"
        local_compiler="${local_compiler:-$(which mavkit-protocol-compiler)}"
    fi
    ...
    echo '#!/bin/sh' > $client_dir/bin/mavkit-client
    echo "exec $client \"\$@\"" >> $client_dir/bin/mavkit-client
    chmod +x $client_dir/bin/mavkit-client

    echo '#!/bin/sh' > $client_dir/bin/mavkit-admin-client
    echo "exec $admin_client \"\$@\""  >> $client_dir/bin/mavkit-admin-client
    chmod +x $client_dir/bin/mavkit-admin-client
    ...
    for protocol in $(cat $bin_dir/../../script-inputs/active_protocol_versions); do
        ...
        echo '#!/bin/sh' > $client_dir/bin/mavkit-baker-$protocol_without_number
        echo "exec $baker \"\$@\""  >> $client_dir/bin/mavkit-baker-$protocol_without_number
        chmod +x $client_dir/bin/mavkit-baker-$protocol_without_number


        echo '#!/bin/sh' > $client_dir/bin/mavkit-smart-rollup-node-$protocol_without_number
        echo "exec $sc_rollup_node \"\$@\" -data-dir $rollup_node_dir"  >> $client_dir/bin/mavkit-smart-rollup-node-$protocol_without_number
        chmod +x $client_dir/bin/mavkit-smart-rollup-node-$protocol_without_number

        echo '#!/bin/sh' > $client_dir/bin/mavkit-smart-rollup-client-$protocol_without_number
        echo "exec $sc_rollup_client \"\$@\""  >> $client_dir/bin/mavkit-smart-rollup-client-$protocol_without_number
        chmod +x $client_dir/bin/mavkit-smart-rollup-client-$protocol_without_number

        echo '#!/bin/sh' > $client_dir/bin/mavkit-accuser-$protocol_without_number
        echo "exec $accuser \"\$@\""  >> $client_dir/bin/mavkit-accuser-$protocol_without_number
        chmod +x $client_dir/bin/mavkit-accuser-$protocol_without_number
    done

    echo '#!/bin/sh' > $client_dir/bin/mavkit-signer
    echo "exec $signer \"\$@\""  >> $client_dir/bin/mavkit-signer
    chmod +x $client_dir/bin/mavkit-signer

    cat <<EOF
if type mavkit-client-reset >/dev/null 2>&1 ; then mavkit-client-reset; fi ;
PATH="$client_dir/bin:\$PATH" ; export PATH ;
alias mavkit-activate-alpha="$client  -block genesis activate protocol ProtoALphaALphaALphaALphaALphaALphaALphaALphaDdp3zK with fitness 1 and key activator and parameters $parameters_file" ;
alias mavkit-client-reset="rm -rf \"$client_dir\"; unalias mavkit-activate-alpha mavkit-client-reset" ;
alias mavkit-autocomplete="if [ \$ZSH_NAME ] ; then autoload bashcompinit ; bashcompinit ; fi ; source \"$bin_dir/bash-completion.sh\"" ;
trap mavkit-client-reset EXIT ;
