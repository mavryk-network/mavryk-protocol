...
let gt_diff diff new_b old_b =
  let diff = Q.of_int64 @@ Tez.to_mumav diff in
  ...
  --> check_snapshot_balances
        ~f:
          (assert_balance_evolution
             ...
             ~part:`staked
             (gt_diff @@ Tez.of_mumav 100_000_000L))
        "before delegation"
         ...
         --> check_snapshot_balances
               ~f:
                 (assert_balance_evolution
                    ...
                    ~for_accounts:[delegate]
                    ~part:`staked
                    (lt_diff @@ Tez.of_mumav 100_000_000L))
               "before second delegation"
         --> check_snapshot_balances
               ~f:
                 (assert_balance_evolution
...
let test_overdelegation =
  ...
  --> check_balance_fields
        "delegate"
        ~liquid:(Tez.of_mumav 200_000_000_000L)
        ~staked:(Tez.of_mumav 200_000_000_000L)
        ()
  ...
  --> check_balance_fields
        "delegate"
        ~liquid:(Tez.of_mumav 20_000_000_000L)
        ~staked:(Tez.of_mumav 380_000_000_000L)
        ()
  --> transfer
        "faucet2"
        "delegator_to_fund"
  ...
  --> check_balance_fields
        "delegate"
        ~liquid:Tez.zero
        ~staked:(Tez.of_mumav 400_000_000_000L)
        ()
  --> transfer
        "faucet3"
        "delegator_to_fund"
  ...
  --> check_balance_field
        "delegator_to_fund"
        `Liquid
        (Tez.of_mumav 14_400_000_000_000L)
  --> check_balance_fields
        "delegate"
        ~liquid:Tez.zero
        ~staked:(Tez.of_mumav 400_000_000_000L)
  ...
  --> transfer
        "delegator_to_fund"
        "faucet1"
        (Amount (Tez.of_mumav 7_200_000_000_000L))
  ...
  --> check_balance_fields
        ...
        ~liquid:Tez.zero
        ~staked:(Tez.of_mumav 380_000_000_000L)
        ~unstaked_frozen_total:(Tez.of_mumav 20_000_000_000L)
        ()
