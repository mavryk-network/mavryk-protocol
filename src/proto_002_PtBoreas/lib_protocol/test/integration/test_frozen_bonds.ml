...
(** Testing
    ...
    Invocation: dune exec src/proto_002_PtBoreas/lib_protocol/test/integration/main.exe \
...
let big_random_amount () =
  match Tez.of_mumav (Int64.add 100_000L (Random.int64 1_000_000L)) with
  | None -> assert false
  | Some x -> x

let small_random_amount () =
  match Tez.of_mumav (Int64.add 1_000L (Random.int64 10_000L)) with
  | None -> assert false
  | Some x -> x
...
let test_delegate_then_freeze_deposit () =
  ...
  let* () = Assert.equal_mav ~loc:__LOC__ staking_balance' staking_balance in
  (* Remove delegation. *)
  ...
  let* () =
    Assert.equal_mav ~loc:__LOC__ staking_balance''' staking_balance''
  in
  ...
  Assert.equal_mav ~loc:__LOC__ user_balance' user_balance
...
let test_total_stake ~user_is_delegate () =
  ...
  let*@ ctxt, balance = Token.Internal_for_tests.balance ctxt user_account in
  let* () = Assert.equal_mav ~loc:__LOC__ (stake -! balance) frozen_bonds in
  let* () =
    Assert.equal_mav ~loc:__LOC__ (stake -! balance) (deposit_amount *! 2L)
  in
  ...
  let*@ frozen_bonds = Contract.get_frozen_bonds ctxt user_contract in
  let* () = Assert.equal_mav ~loc:__LOC__ (stake -! balance) frozen_bonds in
  let* () = Assert.equal_mav ~loc:__LOC__ (stake -! balance) deposit_amount in
  ...
  let*@ stake = Contract.get_balance_and_frozen_bonds ctxt user_contract in
  Assert.equal_mav ~loc:__LOC__ stake balance
...
let check_delegated_balance_is ctxt ~loc delegate expected_balance =
  ...
  Assert.equal_mav ~loc delegated_balance expected_balance
...
let test_rpcs () =
  ...
  let* () = Assert.equal_mav ~loc:__LOC__ frozen_bonds Tez.zero in
  ...
  Assert.equal_mav ~loc:__LOC__ balance_and_frozen_bonds balance
...
let test_scenario scenario =
  ...
  let do_freeze ?(deposit_account = deposit_account1) ctxt =
    ...
    (* Ensure staking balance did not change. *)
    let* () =
      Assert.equal_mav ~loc:__LOC__ staking_balance1' staking_balance1
    in
    let+ () =
      Assert.equal_mav ~loc:__LOC__ staking_balance2' staking_balance2
    in
    ctxt
  in
  ...
  let do_undelegate ?(delegate = delegate1) ctxt amount =
    ...
    let* () =
      Assert.equal_mav ~loc:__LOC__ staking_balance' (staking_balance -! amount)
    in
    (* Fetch user's balance again. *)
    let*@ _, user_balance' =
      Token.Internal_for_tests.balance ctxt user_account
    ...
    let+ () = Assert.equal_mav ~loc:__LOC__ user_balance' user_balance in
    ctxt
  in
