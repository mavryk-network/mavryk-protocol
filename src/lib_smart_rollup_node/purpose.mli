...
(** [make_operator ?default needed_purpose operators] constructs a
    purpose map from a list of bindings [operators], with a potential
    [default] key. If [operators] does not contains all purpose of
    [needed_purpose] or does not contains a [default] key then
    fails.*)
val make_operator :
  ?default_operator:Signature.public_key_hash ->
  needed_purpose:t list ->
  (t * Signature.public_key_hash) list ->
  operators tzresult

(** [replace_operator ?default_operator ~needed_purpose purposed_key
    operators] replaces keys of [operators] by [ default_operator] if
    it's set or by the purposed key in [purposed_key]. It does nothing
    if [default_operator] is not set or [purposed_key] is
    empty. Similar to {!make_operator} the returns [operators] contains
    only mapping of purpose from needed_purpose.*)
