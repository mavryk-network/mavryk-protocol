...
let new_operator_for_purpose :
    type kind. kind purpose_kind -> Signature.public_key_hash -> ex_operator =
 fun purpose pkh ->
  match purpose with
  | Operating -> Operator (Single pkh)
  | Batching -> Operator (Single pkh)
  | Cementing -> Operator (Single pkh)
  | Recovering -> Operator (Single pkh)
  | Executing_outbox -> Operator (Single pkh)
...
let update :
 ...
 fun purpose pkh -> function
  ...
  | None -> Some (new_operator_for_purpose purpose pkh)
      ...
      (fun (Purpose purpose) replacement_operator existing_operator ->
        ...
        | _, existing_operator ->
            let default_operator =
              Option.map (new_operator_for_purpose purpose) default_operator
            in
