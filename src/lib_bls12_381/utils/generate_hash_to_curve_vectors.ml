...
let () =
  ...
  let output_g1 = Mavryk_bls12_381.G1.hash_to_curve msg dst in
  let output_g2 = Mavryk_bls12_381.G2.hash_to_curve msg dst in
  Printf.printf "Msg = %s\n" Hex.(show (of_bytes msg)) ;
  Printf.printf "Dst = %s\n" Hex.(show (of_bytes dst)) ;
  Printf.printf
    "hash_to_curve_g1(msg, dst) = %s\n"
    Hex.(show (of_bytes (Mavryk_bls12_381.G1.to_bytes output_g1))) ;
  Printf.printf
    "hash_to_curve_g2(msg, dst) = %s\n"
    Hex.(show (of_bytes (Mavryk_bls12_381.G2.to_bytes output_g2)))
