...
let pow_one_on_random_element_equals_the_random_element () =
  let e = Mavryk_bls12_381.Fq12.random () in
  assert (Mavryk_bls12_381.Fq12.eq (Mavryk_bls12_381.Fq12.pow e Z.one) e)

(** x**(-n) = x**(g - 1 - n) where g is the order of the additive group *)
let pow_to_negative_exponent () =
  let x = Mavryk_bls12_381.Fq12.random () in
  let n = Z.of_int (Random.int 1_000_000_000) in
  assert (
    Mavryk_bls12_381.Fq12.eq
      (Mavryk_bls12_381.Fq12.pow x (Z.neg n))
      (Mavryk_bls12_381.Fq12.pow x (Z.sub (Z.pred order) n)))

(** x**g = x where g = |(F, +, 0)| *)
let pow_to_the_additive_group_order_equals_same_element () =
  let x = Mavryk_bls12_381.Fq12.random () in
  assert (Mavryk_bls12_381.Fq12.eq (Mavryk_bls12_381.Fq12.pow x order) x)

(** x**g = 1 where g = |(F, *, 1)| *)
let rec pow_to_the_multiplicative_group_order_equals_one () =
  let x = Mavryk_bls12_381.Fq12.random () in
  if Mavryk_bls12_381.Fq12.is_zero x then
    pow_to_the_multiplicative_group_order_equals_one ()
  else
    assert (
      Mavryk_bls12_381.Fq12.eq
        (Mavryk_bls12_381.Fq12.pow x (Z.pred order))
        Mavryk_bls12_381.Fq12.one)
...
let pow_add_multiplicative_group_order_to_a_random_power () =
  ...
  assert (
    Mavryk_bls12_381.Fq12.eq
      (Mavryk_bls12_381.Fq12.pow x (Z.add n order))
      (Mavryk_bls12_381.Fq12.pow x n))

let test_is_one_with_random () =
  assert (not (Mavryk_bls12_381.Fq12.is_one (Mavryk_bls12_381.Fq12.random ())))

let test_is_one_with_zero () =
  assert (not (Mavryk_bls12_381.Fq12.is_one Mavryk_bls12_381.Fq12.zero))

let test_is_one_with_gt_generator () =
  assert (
    not
      (Mavryk_bls12_381.Fq12.is_one
         Mavryk_bls12_381.Fq12.(of_bytes_exn Mavryk_bls12_381.GT.(to_bytes one))))

let test_is_one_with_gt_zero () =
  assert (
    Mavryk_bls12_381.Fq12.is_one
      Mavryk_bls12_381.Fq12.(of_bytes_exn Mavryk_bls12_381.GT.(to_bytes zero)))

let test_is_zero_with_random () =
  assert (not (Mavryk_bls12_381.Fq12.is_zero (Mavryk_bls12_381.Fq12.random ())))

let test_is_zero_with_zero () =
  assert (Mavryk_bls12_381.Fq12.is_zero Mavryk_bls12_381.Fq12.zero)

let test_is_zero_with_gt_generator () =
  assert (
    not
      (Mavryk_bls12_381.Fq12.is_zero
         Mavryk_bls12_381.Fq12.(of_bytes_exn Mavryk_bls12_381.GT.(to_bytes one))))
...
let test_is_zero_with_gt_zero () =
  assert (
    not
      (Mavryk_bls12_381.Fq12.is_zero
         Mavryk_bls12_381.Fq12.(
           of_bytes_exn Mavryk_bls12_381.GT.(to_bytes zero))))
