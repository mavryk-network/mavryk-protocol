let test_fr () =
  let expected_value =
    if Utils.is_ocaml_5 then
      Mavryk_bls12_381.Fr.of_string
        "22989950997422305335515836398089845873697376379127775848923176217833397845914"
    else
      Mavryk_bls12_381.Fr.of_string
        "14994049377928826363495223704381650737411020059923663878974688106580230994440"
  in
  let state = Random.State.make [|42|] in
  let x = Mavryk_bls12_381.Fr.random ~state () in
  if not @@ Mavryk_bls12_381.Fr.(eq x expected_value) then
    Alcotest.failf
      "Expected value is %s but the generated random value is %s"
      (Mavryk_bls12_381.Fr.to_string expected_value)
      (Mavryk_bls12_381.Fr.to_string x)

let test_fq12 () =
  let state = Random.State.make [|42|] in
  let x = Mavryk_bls12_381.Fq12.random ~state () in
  let expected_value_str =
    if Utils.is_ocaml_5 then
      ""
    else
      ""
  in
  let expected_value =
    Mavryk_bls12_381.Fq12.of_bytes_exn (Hex.to_bytes (`Hex expected_value_str))
  in
  if not @@ Mavryk_bls12_381.Fq12.(eq x expected_value) then
    Alcotest.failf
      "Expected value is %s but the generated random value is %s"
      Hex.(show (of_bytes (Mavryk_bls12_381.Fq12.to_bytes expected_value)))
      Hex.(show (of_bytes (Mavryk_bls12_381.Fq12.to_bytes x)))

let test_g1 () =
  let state = Random.State.make [|42|] in
  let x = Mavryk_bls12_381.G1.random ~state () in
  let expected_value_str =
    if Utils.is_ocaml_5 then
      "0b88058e33258eebb07cc801e81ca0ef4a1c721a03bcd7dd8c9cca912026eb61e54081c0504d267cf0cee5eeb5eda0ee0dc4b1a1dc1ad6cd2adbc8582b1142ee96a6cda2f8df0c7c287c9df765f9476d8392a794ed4251fc8f0347fecc2009ca"
    else
      "00e9a2b6bb8b06be174a7d3cbe03e8f4bf2371149124ebb2d11dba1364c588705871df293ef1be2be7f93638b004c3a806993a04535895a92a05e37dedcc0819a7362475eab3befd786569aa9f915a4179f1d60eb0437665dc13dcb2084bc3ef"
  in
  let expected_value =
    Mavryk_bls12_381.G1.of_bytes_exn (Hex.to_bytes (`Hex expected_value_str))
  in
  if not @@ Mavryk_bls12_381.G1.(eq x expected_value) then
    Alcotest.failf
      "Expected value is %s but the generated random value is %s"
      Hex.(show (of_bytes (Mavryk_bls12_381.G1.to_bytes expected_value)))
      Hex.(show (of_bytes (Mavryk_bls12_381.G1.to_bytes x)))

let test_g2 () =
  let state = Random.State.make [|42|] in
  let x = Mavryk_bls12_381.G2.random ~state () in
  let expected_value_str =
    if Utils.is_ocaml_5 then
      ""
    else
      ""
  in
  let expected_value =
    Mavryk_bls12_381.G2.of_bytes_exn (Hex.to_bytes (`Hex expected_value_str))
  in
  if not @@ Mavryk_bls12_381.G2.(eq x expected_value) then
    Alcotest.failf
      "Expected value is %s but the generated random value is %s"
      Hex.(show (of_bytes (Mavryk_bls12_381.G2.to_bytes expected_value)))
      Hex.(show (of_bytes (Mavryk_bls12_381.G2.to_bytes x)))

let () =
  let open Alcotest in
  run
    ~__FILE__
    "Random state"
    [
      ( "Random state",
        [
          test_case "Fr" `Quick test_fr;
          test_case "Fq12" `Quick test_fq12;
          test_case "G1" `Quick test_g1;
          test_case "G2" `Quick test_g2;
        ] );
    ]
