module Stubs = struct
  ...
  external anemoi_allocate_ctxt :
    ...
    constants:Mavryk_bls12_381.Fr.t array ->
    beta:Mavryk_bls12_381.Fr.t ->
    delta:Mavryk_bls12_381.Fr.t ->
    int ->
    int ->
    ctxt
...
module Parameters = struct
  ...
  let generate_constants nb_rounds l =
    let csts_c =
      Array.init (l * nb_rounds) (fun _ -> Mavryk_bls12_381.Fr.(copy zero))
    in
    let csts_d =
      Array.init (l * nb_rounds) (fun _ -> Mavryk_bls12_381.Fr.(copy zero))
    in
    for i = 0 to nb_rounds - 1 do
      let pi_0_i = Mavryk_bls12_381.Fr.pow pi_0 (Z.of_int i) in
      let pi_0_i_square = Mavryk_bls12_381.Fr.(square pi_0_i) in
      for j = 0 to l - 1 do
        let pi_1_j = Mavryk_bls12_381.Fr.pow pi_1 (Z.of_int j) in
        let pi_1_j_square = Mavryk_bls12_381.Fr.square pi_1_j in
        csts_c.((i * l) + j) <-
          Mavryk_bls12_381.Fr.(
            (g * pi_0_i_square)
            + pow (pi_0_i + pi_1_j) (Mavryk_bls12_381.Fr.to_z alpha)) ;
        csts_d.((i * l) + j) <-
          Mavryk_bls12_381.Fr.(
            (g * pi_1_j_square)
            + pow (pi_0_i + pi_1_j) (Mavryk_bls12_381.Fr.to_z alpha)
  ...
  let security_128_state_size_2 =
    {
      ...
      nb_rounds = 19;
      state_size = 2;
      linear_layer = Mavryk_bls12_381.Fr.[|[|one; g|]; [|g; square g + one|]|];
...
let jive128_1 x y =
  ...
  Mavryk_bls12_381.Fr.(x + y + state.(0) + state.(1))
