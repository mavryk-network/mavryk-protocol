...
pub enum Error {
    ...
    /// authorized limit.
    StoreKeyTooLarge = mavryk_smart_rollup_core::STORE_KEY_TOO_LARGE,
    /// The store key submitted as an argument of a host function cannot be
    /// parsed.
    StoreInvalidKey = mavryk_smart_rollup_core::STORE_INVALID_KEY,
    /// The contents (if any) of the store under the key submitted as an
    /// argument of a host function is not a value.
    StoreNotAValue = mavryk_smart_rollup_core::STORE_NOT_A_VALUE,
    /// An access in a value of the durable storage has failed, supposedly out
    /// of bounds of a value.
    StoreInvalidAccess = mavryk_smart_rollup_core::STORE_INVALID_ACCESS,
    /// Writing a value has exceeded 2^31 bytes.
    StoreValueSizeExceeded = mavryk_smart_rollup_core::STORE_VALUE_SIZE_EXCEEDED,
    /// An address is out of bound of the memory.
    MemoryInvalidAccess = mavryk_smart_rollup_core::MEMORY_INVALID_ACCESS,
    /// The input or output submitted as an argument of a host function exceeds
    /// the authorized limit.
    InputOutputTooLarge = mavryk_smart_rollup_core::INPUT_OUTPUT_TOO_LARGE,
    /// Generic error code for unexpected errors.
    GenericInvalidAccess = mavryk_smart_rollup_core::GENERIC_INVALID_ACCESS,
    /// A value cannot be modified if it is readonly.
    StoreReadonlyValue = mavryk_smart_rollup_core::STORE_READONLY_VALUE,
    /// The key was not found in storage
    StoreNotANode = mavryk_smart_rollup_core::STORE_NOT_A_NODE,
    /// The outbox is full
    FullOutbox = mavryk_smart_rollup_core::FULL_OUTBOX,
}
...
impl From<i32> for Error {
    fn from(code: i32) -> Self {
        match code {
            mavryk_smart_rollup_core::STORE_KEY_TOO_LARGE => Self::StoreKeyTooLarge,
            mavryk_smart_rollup_core::STORE_INVALID_KEY => Self::StoreInvalidKey,
            mavryk_smart_rollup_core::STORE_NOT_A_VALUE => Self::StoreNotAValue,
            mavryk_smart_rollup_core::STORE_VALUE_SIZE_EXCEEDED => {
