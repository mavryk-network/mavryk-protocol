...
extern "C" {
    ...
    /// `payload_len` is the same as the one used by the Mavryk protocol.
    ///
...
mod riscv64_hermit {
    ...
    #[inline(always)]
    unsafe fn sbi_mavryk_inbox_next(buffer: *mut u8, max_len: u64) -> MessageInfo {
        let level: i32;
        let id: i32;
        let length: u64;
    ...
    #[inline(always)]
    unsafe fn sbi_mavryk_meta_origination_level() -> u64 {
        let level: u64;
    ...
    #[inline(always)]
    unsafe fn sbi_mavryk_meta_address(buffer: *mut u8, max_len: u64) -> u64 {
        let result: u64;
    ...
    pub unsafe fn read_input(
        ...
        let info = sbi_mavryk_inbox_next(dst, max_bytes as u64);
    ...
    pub unsafe fn reveal_metadata(buffer: *mut u8, max_bytes: usize) -> i32 {
        ...
        let offset = sbi_mavryk_meta_address(buffer, max_bytes as u64) as usize;

        // Fill the remaining part of the buffer with the origination level.
        let orig_level = (sbi_mavryk_meta_origination_level() as u32).to_le_bytes();
