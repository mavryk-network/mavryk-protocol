...
- Add `Runtime::reveal_dal_page` to let a kernel request pages from Mavryk’ Data
...
- Add `KERNEL_BOOT_PATH` constant to `mavryk-smart-rollup-host`.
- `mavryk-smart-rollup-installer-config`: add `eval_config_instr` and `upgrade_reveal_flow` functions to
...
- Add `inbox::ExternalMessageFrame` to `mavryk-smart-rollup-encoding`, to define a shared framing protocol for
...
- Introduce `PublicKey` definition in `mavryk-smart-rollup-encoding`
...
Initial release to [crates.io](https://crates.io/crates/mavryk-smart-rollup), of the following:

- `mavryk-smart-rollup-core`: low-level C-style definitions & linking of host functions.
- `mavryk-smart-rollup-host`: safe-rust wrapper trait & definitions over host functions.
- `mavryk-smart-rollup-debug`: output formatted messages to the rollup debug log.
- `mavryk-smart-rollup-panic-hook`: capture kernel panics & write them to the debug log.
- `mavryk-smart-rollup-encoding`: rollup-specific data types & encodings, such as definitions
  for parsing inbox messages, and writing outbox messages.
- `mavryk-smart-rollup-storage`: transactional account system over durable storage, supporting
  nested commits/rollbacks of account state.
- `mavryk-smart-rollup-entrypoint`: macro to correctly initialise `kernel_entry` entrypoint when
  targetting WASM, which is called by the rollup.
- `mavryk-smart-rollup-mock`: mock implementation of host functions, to enable unit/integration
  testing of kernels without needing to compile to WASM.
- `mavryk-smart-rollup-sdk`: top-level SDK crate to allow kernel development with fewer initial
  dependencies.
...
Initial release to [crates.io](https://crates.io/crates/mavryk-smart-rollup-installer).
