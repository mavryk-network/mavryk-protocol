...
The test kernels have been built from [mavryk-network/kernel](https://gitlab.com/trili/kernel.git), then manually edited to take into account a change in naming introduced by [this MR](https://gitlab.com/tezos/tezos/-/merge_requests/6914).
...
Next, clone the *mavryk-network/kernel* repository:
``` shell
...
To build the `computation.wasm` kernel, run the following from the checked-out `mavryk-network/kernel` repo:
``` shell
git checkout mavryk-computation
...
To build, run the following from the checked-out `mavryk-network/kernel` repo:
```shell
