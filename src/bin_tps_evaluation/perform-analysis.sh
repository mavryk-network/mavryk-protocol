...
docker run -d --name mavryk_history_analysis -p 5432:5432 -e POSTGRES_HOST_AUTH_METHOD=trust -e POSTGRES_USER=tezos postgres:14-alpine
bunzip2 -c "$DATABASE_DUMP_FILE" | docker exec -i mavryk_history_analysis psql -U tezos -a tezos
../../mavryk-tps-evaluation-estimate-average-block -a connection-string=postgresql://tezos:tezos@localhost:5432 -a start-date="$START_DATE" -a end-date="$END_DATE" > "$RESULT_FILE"
