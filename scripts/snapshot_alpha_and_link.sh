...
$ ./scripts/snapshot_alpha_and_link.sh [<status> [<version_number> [<name>]]]
...
<status> should be either 'active' (default), 'frozen', 'overriden' or
'not_mainnet'. See 'manifest/main.ml' for more information on
<status>.  <version_number> defaults to the the last snapshotted
protocol number plus one. <name> defaults to 'next'.  "
...
"$script_dir"/link_protocol.sh src/proto_"${version_number}"_"${short_hash}" "$status"
