...
check_history() {
    ...
    if git log --format="%s" "${merge_base}"..HEAD | grep -q -f "${pattern_file}" > /dev/null; then
        ...
        for commit in $(git log --format="%H" "${merge_base}"..HEAD); do
            ...
            while read -r pattern; do
                if echo  "${commit_title}" | grep -q "$pattern"; then
                    echo "/!\ Commit ${commit} with title:"
                    echo "> '${commit_title}'"
                    echo "matches blacklist pattern '${pattern}', please revise."
                fi
            done < "${script_dir}forbidden_commit_messages.txt"
        done
