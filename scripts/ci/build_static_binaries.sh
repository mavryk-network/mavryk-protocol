...
cat $EXECUTABLE_FILES |
  while read -r executable; do
    ...
    mv "$executable" "mavkit-binaries/$ARCH/$executable"
    # Write access is needed by strip below.
    chmod +w "mavkit-binaries/$ARCH/$executable"
  done

echo 'Check mavkit-client --version'
SHA=$(git rev-parse --short=8 HEAD)
client_version=$("mavkit-binaries/$ARCH/mavkit-client" --version | cut -f 1 -d ' ')
if [ "$SHA" != "$client_version" ]; then
  echo "Unexpected version for mavkit-client (expected $SHA, found $client_version)"
  exit 1
fi
echo "mavkit-client --version returned the expected commit hash: $SHA"

echo "Strip debug symbols and compress binaries (parallelized)"
# shellcheck disable=SC2046,SC2038
find "mavkit-binaries/$ARCH" -maxdepth 1 -type f ! -name "*.*" | xargs -n1 -P$(nproc) -i sh -c 'strip --strip-debug {}; upx -6q {};'

# Show the effect of previous actions
find "mavkit-binaries/$ARCH" -maxdepth 1 -type f ! -name "*.*" |
  while read -r b; do
    file "$(realpath "$b")"
  done
