...
if ! opam install opam/virtual/mavkit-deps.opam --deps-only --with-test --show-actions | grep "Nothing to do." > /dev/null 2>&1 ; then
    echo
    echo 'Failure! Missing actions:'
    echo
    opam install opam/virtual/mavkit-deps.opam --deps-only --with-test --show-actions
    echo
