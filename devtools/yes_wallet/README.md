...
following script gets all known aliases from active delegates in Mavryk
Mainnet using [tzkt.io](https://tzkt.io)'s API:

```shell
curl https://api.mavryk.network/v1/delegates?limit=5000 | jq 'map(select ((.alias?) and .active) |{ "alias" : .alias, "address" : .address , "publicKey" : .publicKey})' > aliases.json
