...
module Key = struct
  type t = 'a constraint 'a list = Mavryk_protocol_environment.Context.key
...
module TreeAbstraction (KeySet : KeySet) (AConf : AbstractionConfig) = struct
  ...
  let rec abstract_tree tree =
    ...
    let* value_opt = Mavryk_protocol_environment.Context.Tree.find tree [] in
    match value_opt with
    | Some v ->
        let value_size_in_bytes = Bytes.length v in
        return {empty with value_size_in_bytes}
    | None ->
        let* subtrees = Mavryk_protocol_environment.Context.Tree.list tree [] in
...
module Commands = struct
  let du ~genesis ~precise ~star_threshold ~data_dir =
    ...
    let* proto_hash = Mavryk_store.Store.Block.protocol_hash chain_store head in
    ...
    (* Getting tree root *)
    let*! tree = Mavryk_protocol_environment.Context.find_tree ctxt [] in
...
let string_parameter =
  Mavryk_clic.parameter (fun _ctx s -> Lwt_result_syntax.return s)

let data_dir_param =
  Mavryk_clic.param ~name:"data-dir" ~desc:"Path to context" string_parameter
...
let precision_arg =
  Mavryk_clic.switch ~doc:"Use high-precision of keysets" ~long:"precise" ()
  ...
  | Error trace ->
      Format.printf
        ...
        (Mavryk_clic.pp_cli_errors
           ...
           ~global_options:Mavryk_clic.no_options
